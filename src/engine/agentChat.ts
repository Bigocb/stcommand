...
export interface ChatAgentContext {
  ...
  fleet?: FleetManager;
  api?: SpaceTradersAPI;
  /** Bound once, like Doctrine/MissionManager/FleetManager — see
   *  doctrine.ts's class doc comment for the full reasoning. Only the two
   *  tenant-scoped store reads (recentActivity, ledgerTotals) need it; every
   *  other tool reads shared galaxy data or goes through `ctx.fleet`, which
   *  already carries its own tenantId. */
  tenantId?: string;
}
    ...
    return [
      ...
      },
      {
        ...
        execute: async ({ system, good }) => {
          ...
          let trades = await ctx.store.bestTrades(system ? String(system) : undefined);
      ...
      },
      {
        ...
        execute: async ({ system, good }) => {
          if (!ctx.store) return JSON.stringify({ error: "store not available" });
          let snaps = await ctx.store.latestMarketSnapshots();
      ...
      },
      {
        ...
        execute: async () => {
          if (!ctx.store) return JSON.stringify({ error: "store not available" });
          const snaps = await ctx.store.latestMarketSnapshots();
      ...
      },
      {
        ...
        execute: async ({ good }) => {
          ...
          return JSON.stringify(await ctx.store.goodPriceHistory(String(good), since));
        },
      },
      {
        name: "get_activity",
        ...
        readOnly: true,
        execute: async ({ ship }) => {
          if (!ctx.store || !ctx.tenantId) return JSON.stringify({ error: "store not available" });
          let activity = await ctx.store.recentActivity(ctx.tenantId, 50);
      ...
      },
      {
        ...
        execute: async () => {
          if (!ctx.store || !ctx.tenantId) return JSON.stringify({ error: "store not available" });
          return JSON.stringify(await ctx.store.ledgerTotals(ctx.tenantId));
        },
      },
      {
        name: "get_missions",
        ...
        execute: async () => {
          if (!ctx.fleet) return JSON.stringify({ error: "fleet not ready" });
          return JSON.stringify(await ctx.fleet.getMissions());
        },
      },
      {
